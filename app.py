...
if clicked:
    ...
    for csym in cryptos:
        if not cmc_api_key:
            st.markdown(
                f"<div class='warning'>API Key required for {csym} from CoinMarketCap!</div>",
                unsafe_allow_html=True
            )
            st.stop()
    ...
    if not usable_symbols:
        st.markdown(
            f"<div class='warning'>No valid symbols/data. Are markets open? Typo in names? API working?</div>",
            unsafe_allow_html=True
        )
        st.stop()
    ...
    st.markdown(
        ...
        + " ".join([
            ...
            + (f"<span style='color:{safe};font-weight:700;'>🟩 ${fmt_price(live_quotes[s])}</span>"
               if isnumber_finite(live_quotes[s]) else
    ...
    st.dataframe(trx_df[trx_df['compliance_breaches'].apply(len) > 0][
        ['timestamp', 'trader_id', 'symbol', 'amount', 'price', 'leverage', 'compliance_breaches']
